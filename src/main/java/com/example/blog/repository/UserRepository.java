...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query("SELECT new com.example.blog.dto.user.UserDTO(u.email, u.username, u.firstName, u.lastName, u.bio, " +
            "new com.example.blog.dto.user.SocialLinksDTO(u.socialLinks.youtube, u.socialLinks.instagram, u.socialLinks.facebook, u.socialLinks.twitter, u.socialLinks.github, u.socialLinks.website), " +
            "u.createdOn, u.profileImage) " +
            "FROM User u WHERE u.username like %:searchType% or u.firstName like %:searchType% or u.lastName like %:searchType% or u.email like %:searchType%")
    List<UserDTO> getUserBySearchType(@Param("searchType") String searchType);


    @Modifying
    @Transactional
    @Query(value = "update users set profile_image = :image where user_id= :userId", nativeQuery = true)
    void uploadImage(@Param("image") String image,@Param("userId") long userId);
}
